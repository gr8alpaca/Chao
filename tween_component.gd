@tool
class_name TweenComponent extends ReferenceRect
...
func show_delayed(time_sec: float = 0.0) -> void:
	enabled = true
	set_process(false)
	if time_sec > 0.0: 
		create_tween().tween_callback(set_process.bind(true)).set_delay(time_sec)
...
func _get_minimum_size() -> Vector2:
	...
	if is_inside_tree():
		min_size.x = maxf(min_size.x, get_parent().get_minimum_size().x)
		min_size.y = maxf(min_size.y, get_parent().get_minimum_size().y)
	return min_size
...
func get_controls() -> Array[Control]:
	...
	controls.push_back(get_parent() as Control)
	return controls
